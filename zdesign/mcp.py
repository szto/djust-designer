"""zdesign MCP stdio server — bridges Claude Code to a running zdesign app.
...
Usage
-----
1. Install the extra: ``pip install 'zdesign[mcp]'`` (installs the ``mcp`` SDK).
...
The server assumes the Django app is reachable at ``http://127.0.0.1:8000`` by
default. Override with ``ZDESIGN_URL`` or the ``--url`` flag.
"""
...
try:
    ...
    raise SystemExit(
        "zdesign.mcp requires the `mcp` package. Install it with:\n    pip install 'zdesign[mcp]'"
    ) from exc


BASE_URL = os.environ.get("ZDESIGN_URL", "http://127.0.0.1:8000")
mcp = FastMCP("zdesign")
...
def get_current_selection() -> dict:
    ...
    """
    return _post("/__zdesign__/selection")
...
def resolve_id(zd_id: str) -> dict:
    ...
    """
    return _post("/__zdesign__/resolve", {"zd_id": zd_id})
...
def edit_class(zd_id: str, new_class: str) -> dict:
    ...
    """
    return _post("/__zdesign__/edit/class", {"zd_id": zd_id, "class": new_class})


@mcp.tool()
def undo_last() -> dict:
    """Restore the most recent snapshot taken by ``edit_class``."""
    return _post("/__zdesign__/undo")


def main() -> None:
    global BASE_URL
    parser = argparse.ArgumentParser(description="zdesign MCP stdio server")
    parser.add_argument(
        "--url",
        default=BASE_URL,
        help="Base URL of the running zdesign Django app (default: %(default)s)",
    )
