...
from djust_designer.middleware import DjustDesignerMiddleware


def _mw():
    return DjustDesignerMiddleware(get_response=lambda r: r)
...
def test_injects_assets_into_html_response_when_debug():
    ...
    assert "djust_designer/overlay.js" in body
    assert "djust_designer/overlay.css" in body
...
def test_no_inject_when_debug_false():
    resp = HttpResponse("<html><body>hi</body></html>", content_type="text/html")
    out = _mw().process_response(Mock(), resp)
    assert "djust_designer/overlay.js" not in out.content.decode()
...
def test_idempotent_when_already_injected():
    src = '<html><body>x<script src="/static/djust_designer/overlay.js"></script></body></html>'
    resp = HttpResponse(src, content_type="text/html")
    out = _mw().process_response(Mock(), resp)
    assert out.content.decode().count("djust_designer/overlay.js") == 1
